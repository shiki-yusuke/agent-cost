...
**step 契約（v5、Copilot #1）**: 3 job の全 step は上の列と**順序・個数・種別で完全一致**する。`uses` step は D3 の allowlist 文字列、`run` step は `name` の完全一致 + run スクリプトの**正規化後（行末空白除去・空行除去、コメント行は保持）の完全一致** + `if` の完全一致（version 検証 step のみ `github.event_name == 'push'`、他は `if` 無し）で検査する（RULE-03 / 05 / 12 / 15、lint (j)）。`uses` 列だけの比較では run step（特に version 検証）を削除・空洞化しても lint が通るため。マーカーの部分一致も採らない（terra レビュー: `... || true`、末尾の `true`、コメントに残したマーカーで空洞化できる）。したがって run スクリプトの一切の変更（コメント含む）は spec 改訂事項で、release.yml と release_checks.py の `EXPECTED_STEPS` を同じ変更で更新する。asset 集合検証の bash は `whl=(dist/*.whl)` 等の配列で件数 1 と実在を先に検査し、`expected` / `actual` を `sort` 済みの改行区切り文字列として引用付きで比較する（`ls` の失敗が command substitution 内で隠れる形、未引用の word-splitting を避ける）。
...
- **RULE-15**（v5）workflow-lint は 3 job の全 step を D1 の step 契約（順序・個数・`uses` 文字列・`run` step の name と正規化後スクリプト全文と `if`）と完全一致で検査する。部分一致・マーカー方式は採らない。
